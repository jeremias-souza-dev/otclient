#!/bin/bash
set -euo pipefail
# Variant of setup_android_deps.sh restricted to x86_64 only (emulator ABI)

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
NDK="${ANDROID_NDK_HOME:-$HOME/android-sdk/ndk/29.0.13599879}"
NDKBIN="$NDK/toolchains/llvm/prebuilt/linux-x86_64/bin"
VCPKG="${VCPKG_ROOT:-$HOME/vcpkg}"
...
echo "============================="
echo "Step 1: Building LuaJIT (x86_64 only)"
echo "============================="
...
echo ""
echo "============================="
echo "Step 2: Installing vcpkg deps (x64-android only)"
echo "============================="
for ABI in "${!ABI_MAP[@]}"; do
    TRIPLET=${ABI_MAP[$ABI]}
...
echo ""
echo "Now run: cd android && OTCLIENT_ANDROID_ABIS=x86_64 ./gradlew assembleRelease"
